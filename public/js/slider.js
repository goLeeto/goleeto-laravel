...
$(function(){
	...
	function nextSlide(){					
			TweenLite.to( $slides.eq(currentSlide), slideTime, {autoAlpha:0,left:'-100%'} );		//fade out the old slide
			currentSlide = ++currentSlide % $slides.length;											//find out which is the next slide			
			TweenLite.to( $slides.eq(currentSlide), slideTime, {autoAlpha:1,left:'15%'} );			//fade in the next slide
			TweenLite.set($slides.eq(currentSlide),{left:'100%'});									//set Current Slide to 100% of window
